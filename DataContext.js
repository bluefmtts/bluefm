...
const DataProvider = ({ children }) => {
    ...
    // Background optimization variables (hidden from UI)
    const [allNovelsData, setAllNovelsData] = useState([]);
    const [loadPhase, setLoadPhase] = useState(1); // 1=initial, 2=partial, 3=full
    
    // Load demo data if database is empty - ORIGINAL FUNCTION
    ...
    // Smart Fetch novels from Firebase with background loading
    useEffect(() => {
        loadDemoData();
        ...
        const unsubscribe = db.collection('novels')
            ...
            .onSnapshot(snapshot => {
                ...
                setAllNovelsData(novelsData);
                
                if (loadPhase === 1) {
                    // Phase 1: Show first 4 novels immediately
                    setNovels(novelsData.slice(0, 4));
                    setLoading(false);
                    
                    // Phase 2: Load more after 1 second
                    setTimeout(() => {
                        setNovels(novelsData.slice(0, 8));
                        setLoadPhase(2);
                        
                        // Phase 3: Load all after 3 seconds
                        setTimeout(() => {
    ...
    // Save user data when it changes - ORIGINAL FUNCTION
    useEffect(() => {
    ...
    // Helper functions for better performance (background optimization)
    const getNovelById = (novelId) => {
        return novels.find(n => n.id === novelId) || 
               allNovelsData.find(n => n.id === novelId);
    };
    ...
    return (
        <DataContext.Provider value={{
            ...
            getNovelById,
            searchNovels,
            loadPhase // For components to know loading status
        }}>
            {children}
